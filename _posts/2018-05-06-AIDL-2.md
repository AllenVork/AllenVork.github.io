...
上图为 java 文件的结构，里面有一个 Stub 类，Stub 中还有一个 Proxy 的内部类。我们通过 IAvengerAidlInterface.Stub.asInterface(service) 得到的实际是这个 Proxy 类的实例。通过它调用接口的方法时会调用到 Stub 类中的 onTransact(int code, .Parcel data, Parcel reply, int flags) 方法，它是真正执行相关 aidl 接口的方法。执行完成后将结果装入一个 reply 的 Parcel 中返回给客户端。
## 详解该 java 文件的具体实现
```java
