...
它会调用 View 的 invalidate() 方法来进行刷新，该方法会将该 View 的大小和位置传给 invalidateInternal() 方法，该方法会调用 parent 的 invalidateChild() 方法，该方法会循环调用 parent.invalidateChildInParent 直到 parent 为空。首次 parent 为自身，返回值为自己的 parent。我们再来看下这个方法的流程图：
